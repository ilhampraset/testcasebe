...
type MonthlyReportDto struct {
	Info Info                          `json:"info"`
	Data []domain.MerchantMontlyReport `json:"data"`
}

func NewMerchantHandler(uc domain.IMerchantUsecase, jwt utils.Token) *MerchantHandler {
	return &MerchantHandler{uc, jwt}
}

func (h *MerchantHandler) MonthlyReport(c *gin.Context) {
